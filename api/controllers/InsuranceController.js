...
module.exports = {
  insertAppraisalVal_Ins: function (req, res) {
    ...
    logger.info(log_t);
    request.get({ url: 'http://localhost:1337/logs/create?log=' + (log_t) });
    ...
    Insurance.create({ ins_MortID: ins_MortID, ins_MlSID: ins_MlSID, ins_appraisalValue: incomingAppraisalval }).exec(
      function (err) {
        if (err) {
          ...
          logger.info(log_u);
          request.get({ url: 'http://localhost:1337/logs/create?log=' + (log_u) });
        }
        res.send({ regSuccess: "Application was submitted successfully!" });
        var log_v = 'Application was submitted successfully!';
        logger.info(log_v);
        request.get({ url: 'http://localhost:1337/logs/create?log=' + (log_v) });
      });
    ...
    logger.info(log_w);
    request.get({ url: 'http://localhost:1337/logs/create?log=' + (log_w) });
  },
  ...
  updateInsurance: function (req, res) {
    ...
    logger.info(log_x);
    request.get({ url: 'http://localhost:1337/logs/create?log=' + (log_x) });
    ...
    Insurance.update({ ins_MortID: mortid }).set({ firstname: firstName, lastname: lastName, insured_value: incval, deductible_value: deduval }).exec(function (err) {
      if (err) {
        ...
        logger.info(log_y);
        request.get({ url: 'http://localhost:1337/logs/create?log=' + (log_y) });
      }
    ...
    logger.info(log_z);
    request.get({ url: 'http://localhost:1337/logs/create?log=' + (log_z) });
  }
};